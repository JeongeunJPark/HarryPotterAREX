...
{
    public class HR_ManoUtils : MonoBehaviour
    {
        #region Singleton
        private static HR_ManoUtils instance;
        ...
        public static HR_ManoUtils Instance
        {
            get
            {
                return instance;
            ...
            switch (session.add_on)
            {
                case AddOn.DEFAULT:
                    if (session.orientation == SupportedOrientation.PORTRAIT || session.orientation == SupportedOrientation.PORTRAIT_INVERTED)
                    {
                        ...
                        ratio = (float)HR_ManoManager.Instance.Height / HR_ManoManager.Instance.Width;
                    }
                    else
                    {
                        ratio = (float)HR_ManoManager.Instance.Width / HR_ManoManager.Instance.Height;
        ...
        public void OrientMeshRenderer(MeshRenderer meshRenderer)
        {
            ...
            if (HR_ManoManager.Instance.Manomotion_Session.add_on == AddOn.DEFAULT)
            {
                switch (HR_ManoManager.Instance.Manomotion_Session.orientation)
                {
